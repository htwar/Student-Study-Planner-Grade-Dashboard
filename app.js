...
$('#assignmentForm').addEventListener('submit', (event) => { event.preventDefault(); const id = $('#assignmentId').value; const assignment = { id: id || uid('assignment'), name: $('#assignmentName').value.trim(), courseId: $('#assignmentCourse').value, due: $('#assignmentDue').value, duration: Number($('#assignmentDuration').value), priority: $('#assignmentPriority').value, status: $('#assignmentStatus').value }; if (id) state.assignments = state.assignments.map((item) => item.id === id ? assignment : item); else state.assignments.push(assignment); saveState(id ? 'Assignment updated' : 'Assignment added'); assignmentDialog.close(); render(); showToast(id ? 'Assignment updated' : 'Assignment added'); });
